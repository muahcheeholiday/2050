...
export default function Home() {
        ...
        <div className="relative z-10 min-h-dvh flex flex-col items-center justify-center px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
            <button
              ...
              className="flex-1 px-6 py-4 bg-black/40 border-2 border-orange-500 hover:bg-orange-500 hover:text-black transition-colors backdrop-blur-sm"
            >
  ...
  return (
    <div className="relative min-h-dvh w-full">
      <div
        className="fixed inset-0 bg-cover bg-center transition-all duration-700 -z-10"
        style={{ backgroundImage: `url('${scene.imageUrl}')` }}
      />
      <div className="fixed inset-0 bg-gradient-to-t from-black via-black/70 to-black/20 -z-10" />

      <div
        className="relative z-10 min-h-dvh flex flex-col items-center justify-center px-6 pt-24 sm:px-12 max-w-3xl mx-auto text-center"
      ...
      >
        ...
        <div className="mb-3">
          <span className="inline-block bg-orange-500 text-black font-headline font-semibold text-xs sm:text-sm tracking-wider uppercase px-3 py-1">
            {scene.year}
          </span>
          {isEnding && (
            <span
        ...
        {scene.stat && (
          <div className="border-l-4 border-orange-500 bg-black/50 backdrop-blur-sm px-5 py-4 mb-8 sm:mb-10 mx-auto max-w-xl text-left">
            <p className="font-headline font-bold text-3xl sm:text-4xl text-orange-500 mb-1">
              {scene.stat.value}
            </p>
            ...
            <div className="px-5 py-4">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                {scene.newsFlash.stats.map((s, i) => (
                  <div key={i} className="border-t-2 border-orange-500 pt-2">
                    <p className="font-headline font-bold text-lg sm:text-xl text-orange-500">
                      {s.value}
                    </p>
        ...
        <div className="space-y-3 mx-auto max-w-xl w-full">
          {!isEnding &&
            scene.choices.map((choice, i) => (
              <button
                ...
                className="w-full text-left px-5 py-4 bg-black/40 border-l-4 border-orange-500 hover:bg-black/60 hover:border-orange-400 transition-colors backdrop-blur-sm"
              >
          ...
          {isEnding && (
            <button
              ...
              className={`px-6 py-3 font-headline font-semibold uppercase tracking-wide text-black bg-orange-500 hover:bg-orange-400 transition-colors border-2 ${
      ...
      <div
        className="fixed bottom-0 left-0 right-0 z-20 bg-orange-500 text-black px-6 text-center"
        style={{
          paddingTop: "0.75rem",
